...
def define_cnn(features=None, hparams=None):
  ...
  with slim.arg_scope([slim.conv2d],
    ...
    net = slim.conv2d(net, 128, kernel_size=[11, 11])
    net = slim.max_pool2d(net, kernel_size=[11, 11])
    net = tf.nn.relu(net)
    net = tf.nn.dropout(net,0.4)
    
    net = slim.conv2d(net, 256, kernel_size=[9, 9])
    net = tf.nn.relu(net)
    net = slim.max_pool2d(net, kernel_size=[9,9])
    net = tf.nn.dropout(net,0.4)

    net = slim.conv2d(net, 512, kernel_size=[7,7])
    net = tf.nn.relu(net)
    net = slim.max_pool2d(net, kernel_size=[7,7])
    net = tf.nn.dropout(net,0.4)

    net = slim.conv2d(net, 1024, kernel_size=[5, 5])
    net = tf.nn.relu(net)
    net = slim.max_pool2d(net, kernel_size=[5, 5])
    net = tf.nn.dropout(net,0.4)

    net = slim.conv2d(net, 2048, kernel_size=[3, 3])
    net = tf.nn.relu(net)
